...
The workbench shows this three-step safety guide when no projects are registered. In the macOS desktop app, choose **Help → Replay Getting Started** at any time to review it again without changing projects, trust, or execution state.
...
Project registration is machine-local. Workspace manifests are portable JSONC files under `~/.craft-hub/workspaces/`, while absolute paths and local bindings remain in the operating-system Craft Hub data directory.
