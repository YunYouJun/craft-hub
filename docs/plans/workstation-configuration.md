...
## Initial isolation and dependencies (2026-09-09)
...
- Integration with the main working-copy changes is complete (see the integration record below). The desktop installation has not been replaced.
...
## Initial delivery check results (2026-09-09)
...
## Integration and final acceptance (2026-09-10)

The user authorized merging the implementation and existing working-copy code, resolving conflicts and committing. The original working-copy changes were first preserved in `e63031e` on `codex/integrate-workstation`, then the workstation feature branch was merged. The workstation backend branch was fast-forwarded onto its main branch at `2579076`.

- [x] Resolve 12 conflicted files without discarding the newer resource-page, account, workspace or Codex inspector capabilities.
- [x] Keep the existing typed `configuration` entity adapter; expose workstation review/apply/recovery through `configurationManagement`, sharing operation validation and confirmation. Configuration inspection and mutations use the existing `hostEnvironment` guard, including hosted denial and project trust.
- [x] Register both built-in desktop adapters, retaining explicit distribution overrides by plugin ID.
- [x] Use the current shared workbench header/geometry, Button, Input, FormSelect and theme tokens. Remove the configuration page's independent control styling and global body overrides. Forward accessible labels through the shared FormSelect trigger.
- [x] Craft Hub public-boundary/lint, TypeScript, Vue typecheck, build and full test suite: 140 files / 707 tests passed.
- [x] workstation lint, typecheck, build and all tests: 213 CLI + 9 companion tests passed.
- [x] Rebuild and restart both local review services using the integrated community checkout, replacing the old isolated host.
- [x] In-app browser at port 4327, disposable HOME and repositories: list/diff, shared decision dropdown, MCP preview/apply/synchronized state/backup restore; skills filter/preview/concurrent edit rejection/recovery refresh/fresh preview/apply.
- [ ] Real-machine item decisions remain pending; merging code does not authorize picking a configuration winner or closing the dotfiles sync Todo.
