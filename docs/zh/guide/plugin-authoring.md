...
公开与内部插件都使用同一个 Craft Hub 主程序。宿主扩展提供可执行的 provider，市场插件声明页面与操作。独立品牌的发行包只是可选的打包方式，安装内部能力不需要另装一个应用。

桌面端插件市场提供「宿主扩展」页签。选择作者提供的 `distribution.json`，核对将运行的模块并点击「信任并添加」，重启后生效。此后从 Finder 或 Dock 直接打开 Craft Hub 就会自动加载，无需专用启动器或环境变量。应用品牌、更新配置和项目执行信任不受影响。

连接或启用包含集成声明的本地市场插件时，桌面端也会查找其所在目录中的 `distribution.json`，提供同样的安装流程。发现和确认阶段不执行模块代码；浏览器端不能安装可执行宿主模块。

安装记录保存在操作系统 Craft Hub 数据目录的 `host-extensions.json`，其中保存已确认的模块路径。修改原始清单不会静默增加模块。请保留本地扩展及其依赖的稳定路径；移动目录或调整模块列表后，重新添加即可。在页签中停用或移除后，重启应用以卸载运行中的能力。移除保留源码文件和市场插件；对应宿主扩展停用时，市场插件可能提示提供者不可用。

`craft-hub ui`、`craft-hub app` 的浏览器模式（包括桌面启动失败后的回退）和 `pnpm dev:web` 会在启动时恢复同一份已批准的扩展。`ui --host-plugin` 仍可为当次会话显式加载一个额外的可信模块，不修改安装记录，也不会重复注册同一个模块。CLI 帮助和独立的发现命令不导入已保存扩展；传入自定义 runtime 的嵌入式应用仍自行决定加载哪些 provider。
