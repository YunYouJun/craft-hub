...
describe('workstation host adapter', () => {
  ...
  it('does not allow browser operation selectors to turn a read into a write', async () => {
    ...
    await expect(plugin.integrationProviders![0].configuration!.read({}, { operation: 'apply' })).rejects.toThrow('Unsupported read')
    await expect(plugin.integrationProviders![0].configuration!.update({ confirmed: true }, { operation: 'git-action', action: 'publish' })).rejects.toThrow('Unsupported configuration')
  ...
  it('denies device configuration access in a hosted runtime', async () => {
    ...
    try {
      const call = transport()
      const runtime = new CraftHubRuntime({ dataDir: directory, executionEnvironment: 'hosted', plugins: [createWorkstationPlugin(call)] })
