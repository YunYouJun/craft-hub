...
describe('desktop package scripts', () => {
  ...
  it('ships application icons for the Electron window and macOS package', async () => {
    ...
    expect(packageMacSource).toContain('identity: signingIdentity')
    expect(packageMacSource).toContain('continueOnError: false')
    expect(packageMacSource).toContain('await resolveSigningIdentity()')
    expect(packageMacSource).toContain('\'--strict\',')
  ...
  it('creates architecture-specific DMG installers and ZIP update artifacts', async () => {
    ...
    expect(packageMacSource).toContain('\'log\',')
    expect(packageMacSource).toContain('await signDiskImage(dmgPath, signingIdentity!)')
    expect(packageMacSource).toContain('"Developer ID Application:')
    expect(packageMacSource.indexOf('await signDiskImage(dmgPath, signingIdentity!)')).toBeLessThan(packageMacSource.indexOf('await notarizeArtifact(dmgPath)'))
